...
感谢你愿意为 dsh-context-compass 贡献！本仓库是一个独立单库（2026-09 从 dsh-plugins monorepo 迁出），仓库根即插件目录。
...
npm run smoke                    # 逻辑冒烟（stub 服务）
npm run mount                    # 真实 cordis 挂载测试
node scripts/client-mount.mjs    # 浏览器启动路径
npm run visual                   # Playwright 视觉回归（需运行中 harness）
```
