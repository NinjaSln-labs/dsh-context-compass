...
| **依赖宿主服务的代码按旧契约写**（0.12.2 挖出，潜伏自 0.11.1） | 冷会话 health 永远 null（面板「暂无数据」），无任何报错——`sessionProjectionCache` 两处调用都在 `try/catch` 里抛 `TypeError` 被吞掉。根因：代码写于 0.1.1-rc.2（`cachedSnapshot(meta)` 单参 + `coldSnapshot(id, signal?): Promise`），宿主自 0.1.2-alpha.x 起改为 `cachedSnapshot(meta, inheritedEventCount, keys?)`（第二参经内部 `SessionLogOffset(v)` 校验，缺了必抛）与 `coldSnapshot(meta, count, events): ProjectionSnapshot`（private + 同步 + 调用方自备日志） | **接宿主服务前先读宿主自己的调用点**（`grep -rn "\.cachedSnapshot(" <host>/node_modules/@deepseek-ai/*/lib/index.js` 一眼看出真实实参形状），别照自己的结构类型猜；**fail-soft 路径尤其危险**——异常被 `catch` 吞掉 = 永久静默降级，无日志无症状 |
| **stub 按插件假设写 → 掩盖契约 bug**（0.12.2，pits 2026-09-03 同款重演） | 测试全绿，线上功能全死。原 stub 是 `cachedSnapshot: meta => …` + `coldSnapshot: async …`（照着插件自己的假设写），整整骗过三个版本 | stub 必须复刻**宿主真实契约**：形状 + 参数校验 + 同步/异步语义。本仓 `scripts/tests/helpers.mjs#assertLogOffset` 就是复刻的 `SessionLogOffset` 抛错行为——漏参/错参会让测试当场炸 |
| **`identityMatches` 要求 formatVersion 严格相等** | 修复上线后旧检查点仍读不到（不是 bug）：宿主刻意让「缺 formatVersion」的记录**永不相认**（缺格式代次无法证明折叠语义，其标题走 `cachedPredecessorTitle` 兜底）。当前 `SESSION_FORMAT_VERSION = 3` | 排查「检查点读不到」时先看 `record.identity.formatVersion` 是否存在且等于当前格式版本，再怀疑插件 |
| 视觉门禁：主题竞态 | `panel light` 拍到深色图（比基线失败），但手动点「浅色」是有效的——`setTheme` 在 shell 挂载前读 `body[data-ds-dark-theme]`，读到默认值就误判「已是目标态」提前 return，之后应用才施加持久化主题 | `gotoApp()` 等 shell 就绪；`setTheme` 在切换后**断言目标态真的落定**（不一致直接抛错，不再拍错主题的图去比基线） |
| 视觉门禁：侧栏 markup 随宿主版本变 | badge spec 找不到会话 → `div[class*="sessionRow"]` 为空。0.1.5-rc.1 侧栏默认按工作区分组：会话折叠在 `div.projectRow` 下，需先展开 | `visual/helpers.mjs#revealSessionEntry()` 兼容两种形态（扁平 `sessionRow` / 分组树先展开 `projectRow`）——**宿主侧栏改版时先改这里**，别去改插件 |
